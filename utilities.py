...
ups_color_transparent = [0.8, 0.2, 0.9, 0.01]
...
actual_color_transparent = [0.2, 0.6, 0.2, 0.01]
...
average_district_vote_share_label = "Average district Republican vote share"
...
def add_ups_curve(ax, votes, seats, semi_transparent=False):
    if not semi_transparent:
        ax.step(votes, seats, color=ups_color, linewidth=ups_line_width, zorder=1)
    else:
        ax.step(votes, seats, color=ups_color_transparent, linewidth=ups_line_width, zorder=1)


def add_actual_election(ax, actual_vote_share, actual_seat_share, point_label="", semi_transparent=False):
    if not semi_transparent:
        color = actual_color
    else:
        color = actual_color_transparent
    ax.scatter([actual_vote_share], [actual_seat_share], color=color, edgecolors=color, zorder=3, s=50)
    ax.annotate(point_label, (actual_vote_share, actual_seat_share), textcoords="offset points", xytext=(0, 10),
...
def add_partisan_symmetry(ax, actual_vote_share, actual_seat_share, votes, seats):
    ...
    partisan_symmetry_x = [actual_vote_share - metric_buffer, actual_vote_share + metric_buffer,
    ...
    ax.fill(partisan_symmetry_x, partisan_symmetry_y, '--', color=partisan_symmetry_color,
            edgecolor=partisan_symmetry_color, zorder=2)
...
def plot_ups(actual_vote_share, actual_seat_share, votes, seats, text='', x_label=average_district_vote_share_label):
    fig = plt.figure()
    ax = fig.add_subplot(111)
    add_actual_election(ax, actual_vote_share, actual_seat_share)
    add_ups_curve(ax, votes, seats)
...
def plot_vote_and_seat_shares(vote_share, seat_share, text='', x_label=average_district_vote_share_label):
    fig = plt.figure()
    ax = fig.add_subplot(111)
...
def plot_mean_median_and_partisan_bias_grid(labels, actual_vote_share_list, actual_seat_share_list, vote_list,
                                            seat_list, x_label=average_district_vote_share_label):
    fig, axs = plt.subplots(2, 2, figsize=(10, 10))
    for i in range(0, len(labels)):
        ax = axs[math.floor(i / 2), i % 2]
        rep_vote_share = actual_vote_share_list[i]
...
def plot_eg_band():
    ...
    plt.fill(efficiency_band_x, efficiency_band_y, '--', color=eg_color_transparent, edgecolor=eg_color_transparent,
             zorder=2)
    v_actual = 0.65
    s_actual = 0.65
    ...
    configure_plot(plt, ax, x_label="Vote share")
    ax.set_xlim((0, 1))
    ax.set_ylim((0, 1))


def configure_plot(plt, ax, x_label, text=''):
...
def make_partisan_symmetry_table(labels, actual_vote_share_list, actual_seat_share_list, vote_list, seat_list,
    ...
    if not latex:
        row_format = "{:>15}" * N
        print(row_format.format("Election", "Signed beta average on [0.4, 0.6]", "Unsigned beta average on [0, 1]",
                                "beta(Vo)", "beta(0.5)", "Mean-median"))
    else:
        row_format = "{} & " * (N - 1)
...
def make_competitiveness_table_for_state(republican_votes, democrat_votes, latex=False):
    ...
    for i in range(len(republican_votes)):
        ...
        print(row_format.format(district, total_votes, percent_republican, winner, margin_of_victory_percent,
...
def make_wasted_votes_table_for_state(republican_votes, democrat_votes, latex=False):
    ...
    if not latex:
        row_format = "{:>15}" * (N)
        print(row_format.format("District", "Votes for R", "Votes for D", "Total R + D", "Needed to win", "Wasted by R",
                                "Wasted by D"))
    else:
        row_format = "{} & " * (N - 1)
        row_format = row_format + " {} \\\\ "
    ...
    for i in range(len(republican_votes)):
        ...
        print(
            row_format.format(district, votes_for_r, votes_for_d, total_votes, needed_to_win, wasted_by_r, wasted_by_d))

    total_election_votes = int(np.sum(republican_votes)) + int(np.sum(democrat_votes))

    if not latex:
        print(row_format.format("Total", '{:,}'.format(int(np.sum(republican_votes))),
                                '{:,}'.format(int(np.sum(democrat_votes))), '{:,}'.format(total_election_votes), "",
                                '{:,}'.format(cumulative_wasted_r_votes), '{:,}'.format(cumulative_wasted_d_votes)))
    else:
        print("\\hline")
        print("\\textit{{Total}} & {} & {} & {} & & {} & {}".format('{:,}'.format(int(np.sum(republican_votes))),
                                                                    '{:,}'.format(int(np.sum(democrat_votes))),
                                                                    '{:,}'.format(total_election_votes),
                                                                    '{:,}'.format(cumulative_wasted_r_votes),
                                                                    '{:,}'.format(cumulative_wasted_d_votes)))
        print("\\end{tabular}")
